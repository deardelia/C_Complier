...
//printf("%s %s \n",mid_code[ mi ].op);
	//全局常量
	while(mi<midcode_num){
		while ( strcmp(mid_code[ mi ].op, "const") == 0 ){
printf("global const\n");
			pushstack(mid_code[ mi ].op2,1);
			if(strcmp("int",mid_code[ mi ].op1)==0){
		...
		//全局变量
		while ( strcmp(mid_code[ mi ].op, "int") == 0 || strcmp(mid_code[ mi ].op, "char") == 0 || strcmp(mid_code[ mi ].op, "inta") == 0 || strcmp(mid_code[ mi ].op, "chara") == 0 ) {
			if( strcmp(mid_code[ mi ].op, "int") == 0 || strcmp(mid_code[ mi ].op, "char") == 0){
printf("global var\n");
				pushstack("0",1);
				if (strcmp("int",mid_code[ mi ].op) == 0 ) {
					insertaddress(WINT,-1,-1);
				} else {
					insertaddress(WCHAR,-1,-1);
				}
			...
			else if(strcmp(mid_code[ mi ].op, "inta") == 0 || strcmp(mid_code[ mi ].op, "chara") == 0 ){
printf("global shuzu\n");
				pushstack("0", atoi(mid_code[mi].op2));
				if (strcmp("inta",mid_code[ mi ].op) == 0 ) {
					insertaddress(WINT,-1,-1);
				} else {
					insertaddress(WCHAR,-1,-1);
				}
		...
		constedge = ap;
		//函数定义
		while ( strcmp(mid_code[mi].op, "declare") == 0 || strcmp(mid_code[mi].op, "main") == 0) {
printf("function\n");
		   if ( strcmp(mid_code[ mi ].op1, "char") == 0 ) {
                tfc.kind = CHAR;
                strcpy(tfc.name, mid_code[ mi ].res);
                fc[funcnum]=tfc;
            } else if ( strcmp(mid_code[ mi ].op1, "int") == 0 ) {
                tfc.kind = INT;
                strcpy(tfc.name, mid_code[ mi ].res);
                fc[funcnum]=tfc;
            } else {
            ...
            if ( strcmp(mid_code[ mi ].op, "main") == 0 ) {
                ismain = 1;
...
printf("begin funcasm\n");
    ...
    tmi = mi;
	temp=mi;
	while ( strcmp(mid_code[ tmi ].op, "endp") != 0 ) {//直到函数结束四元式才退出
//printf("%s %s %s %s:\n",mid_code[ mi ].op,mid_code[ mi ].op1,mid_code[ mi ].op2,mid_code[ mi ].res);
 //fprintf(fp_final,"temp;%s %s %s\n",mid_code[ tmi ].op1,mid_code[ tmi ].op2,mid_code[ tmi ].res);
		char v1[100], v2[100], v3[100];
        strcpy(v1, mid_code[ tmi ].op1);
        strcpy(v2, mid_code[ tmi ].op2);
        strcpy(v3, mid_code[ tmi ].res);
        ...
        if ( v3[0] == '$' && varaddr(v3) == -1 ) {
            pushstack("0",1);
            if ( strcmp(mid_code[tmi].op, "call") == 0 ) {
                insertaddress(funckind(mid_code[tmi].op1), -1, tmi);
            }
            else if ( strcmp(mid_code[ tmi ].op, "geta") == 0 ){
...
mi=temp;
	while ( strcmp(mid_code[ mi ].op, "endp") != 0 ) {
//printf("%s %s %s %s:\n",mid_code[ mi ].op,mid_code[ mi ].op1,mid_code[ mi ].op2,mid_code[ mi ].res);
        paran = 0;
        for ( i = 0; i < strlen(mid_code[ mi ].op); i++ ) {
            if ( mid_code[ mi ].op[ i ] == ' ' ) {
                mid_code[ mi ].op[ i ] = '\0';
                break;
            }
        }
        if ( strcmp(mid_code[ mi ].op, "+") == 0 ) addasm();
        if ( strcmp(mid_code[ mi ].op, "-") == 0 ) subasm();
        if ( strcmp(mid_code[ mi ].op, "*") == 0 ) mulasm();
        if ( strcmp(mid_code[ mi ].op, "/") == 0 ) divasm();
        if ( strcmp(mid_code[ mi ].op, ">") == 0 ) greasm();
        if ( strcmp(mid_code[ mi ].op, ">=") == 0 ) geqasm();
        if ( strcmp(mid_code[ mi ].op, "<") == 0 ) lssasm();
        if ( strcmp(mid_code[ mi ].op, "<=") == 0 ) leqasm();
        if ( strcmp(mid_code[ mi ].op, "!=") == 0 ) neqasm();
        if ( strcmp(mid_code[ mi ].op, "==") == 0 ) eqlasm();
        if ( strcmp(mid_code[ mi ].op, "=") == 0 ) assasm();
        if ( strcmp(mid_code[ mi ].op, "[]=") == 0 ) aassasm();
        if ( strcmp(mid_code[ mi ].op, "geta") == 0 ) assaasm();
        if ( strcmp(mid_code[ mi ].op, "lab:") == 0 ) setlabasm();//"lab:", "  ", "  ", label1
        if ( strcmp(mid_code[ mi ].op, "scanf") == 0 ) {
            scfasm();
        }
        if ( strcmp(mid_code[ mi ].op, "printf") == 0 ) {
            prtasm();
        }
        if ( strcmp(mid_code[ mi ].op, "jne") == 0 ) jneasm();
        if ( strcmp(mid_code[ mi ].op, "jmp") == 0 ) jmpasm();//把goto改为jmp?????
        if ( strcmp(mid_code[ mi ].op, "fupa") == 0 ) fupaasm();
        if ( strcmp(mid_code[ mi ].op, "call") == 0 ) callasm();
        if ( strcmp(mid_code[ mi ].op, "return") == 0 ) retasm();
        if ( strcmp(mid_code[ mi ].op, "para") == 0 ) paraasm();
        /*if ( OPTFLAG ) {
            int flag = 0;
            while ( strcmp(mid_code[ mi ].op, "int") == 0
                || strcmp(mid_code[ mi ].op, "char") == 0 ) {
                flag = 1;
                intcharasm();
                mi++;
            }
			...
			if ( strcmp(mid_code[ mi ].op, "int") == 0
                || strcmp(mid_code[ mi ].op, "char") == 0 ) {
						intcharasm();
			 }

        if ( strcmp(mid_code[ mi ].op, "const") == 0 ) constdefasm();
//printf("return const:\n");
//printf("%s %s %s %s:\n",mid_code[ mi ].op,mid_code[ mi ].op1,mid_code[ mi ].op2,mid_code[ mi ].res);
        if ( strcmp(mid_code[ mi ].op, "inta") == 0 || strcmp(mid_code[ mi ].op, "chara") == 0 )
				intcharaasm();
        mi++;
    }
...
printf("insertaddress\n");
    if ( nmi == -1 ) {
        strcpy(addrtable[ ap ].name, mid_code[ mi ].res);
    } else {
        strcpy(addrtable[ ap ].name, mid_code[ nmi ].res);
    }
    if ( addr == -1 ) {
...
printf("pushstack\n");
    if ( lenth == 1 ) {
        fprintf(fp_final, "\t\tli\t$t0\t%s\t#%s\n",item , mid_code[tmi].res ); //li    $t0 item
...
printf("addasm\n");
	...
	if ( isdigit(mid_code[ mi ].op1[ 0 ]) || mid_code[ mi ].op1[ 0 ] == '-' || mid_code[ mi ].op1[ 0 ] == '+' ) {
        fprintf(fp_final,"\t\tli\t$t0\t%s\n" , mid_code[ mi ].op1 );
    } else {
        addr1 = varaddr(mid_code[ mi ].op1);
        if ( isglob )
            fprintf(fp_final,"\t\tlw\t$t0\t%d($t9)\n" , addr1 -2147479548);
        else
            fprintf(fp_final,"\t\tlw\t$t0\t%d($fp)\n" , addr1 );
    }
    if ( isdigit(mid_code[ mi ].op2[ 0 ]) || mid_code[ mi ].op2[ 0 ] == '-' || mid_code[ mi ].op2[ 0 ] == '+' ) {
        fprintf(fp_final, "\t\tli\t$t1\t%s\n" ,mid_code[ mi ].op2 );
    } else {
        addr2 = varaddr(mid_code[ mi ].op2);
        if ( isglob )
            fprintf(fp_final, "\t\tlw\t$t1\t%d($t9)\n" , addr2-2147479548);
        else
            fprintf(fp_final, "\t\tlw\t$t1\t%d($fp)\n" , addr2 );
    }
    addr3 = varaddr(mid_code[ mi ].res);
...
printf("callasm\n");
    ...
    if ( mid_code[ mi ].res[ 0 ] != ' ' && mid_code[ mi ].res[ 0 ] != '\0' ) {
        addr2 = varaddr(mid_code[ mi ].res);
        if ( isglob )
...
void aassasm() {
    ...
    if ( isdigit(mid_code[ mi ].op2[0]) ) {
        addr1 += (atoi(mid_code[ mi ].op2) * 4);//???????????????
        if ( isdigit(mid_code[ mi ].res[ 0 ]) || mid_code[ mi ].res[ 0 ] == '-' || mid_code[ mi ].res[ 0 ] == '+' ) {
            fprintf(fp_final,"\t\tli\t$t0\t%s\n" ,mid_code[ mi ].res);
        } else {
            addr2 = varaddr(mid_code[ mi ].res);
            if ( isglob ) {
        ...
        //求数组元素a[i]地址
        addrt = varaddr(mid_code[ mi ].op2);//addrt = &i
        if (isglob)
...
void assaasm() {//
    ...
    if ( isdigit(mid_code[ mi ].op2[ 0 ]) || mid_code[ mi ].op2[ 0 ] == '-' || mid_code[ mi ].op2[ 0 ] == '+' ) {
        addr1 += ( atoi(mid_code[ mi ].op2) * 4 );    //addr1 = &a[n]
        addr2 = varaddr(mid_code[ mi ].res);        //addr2 = &b
        if ( tisglob ) {
            fprintf(fp_final,"\t\tlw\t$t0\t%d($t9)\n",addr1-2147479548);
        }else
            fprintf(fp_final,"\t\tlw\t$t0\t%d($fp)\n" , addr1 );
...
void scfasm() {
	...
	ti	= findvartable(mid_code[ mi ].res);
	kind	= addrtable[ ti ].kind;
	addr	= varaddr(mid_code[ mi ].res);
...
//printf("%s %s %s %s:\n",mid_code[ mi ].op,mid_code[ mi ].op1,mid_code[ mi ].op2,mid_code[ mi ].res);
    ...
    if ( mid_code[ mi ].op1[ 0 ] != '\0' ) {
        len = strlen(mid_code[ mi ].op1);
        for ( i = 0; i < len; i++ ) {
//printf("%c\n",mid_code[ mi ].op1[i]);
            fprintf(fp_final, "\t\tli\t$v0\t11\n" );
//printf("string:\n");
            fprintf(fp_final, "\t\tli\t$a0\t%d\n" ,(int)(mid_code[ mi ].op1[i]) );
//printf("string:\n");
            fprintf(fp_final, "\t\tsyscall\n" );
        }
    }
//printf("%d\n",mid_code[ mi ].op2[ 0 ] == '\0');
    if ( mid_code[ mi ].op2[ 0 ] != ' ' && mid_code[ mi ].op2[ 0 ] != '\0' ) {
        if ( isdigit(mid_code[mi].op2[0]) && strcmp(mid_code[mi].res, "char") == 0 ) {
            fprintf(fp_final, "\t\tli\t$v0\t11\n" );
            fprintf(fp_final, "\t\tli\t$a0\t%s\n",mid_code[ mi ].op2 );
            fprintf(fp_final, "\t\tsyscall\n" );
            return;
        } else if ( isdigit(mid_code[ mi ].op2[ 0 ]) || mid_code[ mi ].op2[ 0 ] == '-' || mid_code[ mi ].op2[ 0 ] == '+' ) {
            fprintf(fp_final, "\t\tli\t$v0\t1\n" );
            fprintf(fp_final, "\t\tli\t$a0\t%s\n" , mid_code[ mi ].op2 );
            fprintf(fp_final, "\t\tsyscall\n" );
            return;
        }
        addr = varaddr(mid_code[ mi ].op2);
         ti = findvartable(mid_code[ mi ].op2);
         kind = addrtable[ ti ].kind;
        if ( kind == INT || kind == WINT || strcmp(mid_code[ mi ].res,"int")==0) {
            fprintf(fp_final, "\t\tli\t$v0\t1\n" );
            if ( isglob ) {
...
void fupaasm() {
	printf("fupaasm\n");
	int addrt;
    if ( isdigit(mid_code[mi].res[0]) ) {
        fprintf(fp_final, "\t\tli\t$t0\t%s\n" ,mid_code[ mi ].res );
    } else {
		addrt=varaddr(mid_code[ mi ].res);
        if(isglob){
...
void retasm() {
    ...
    if ( mid_code[ mi ].res[ 0 ] != ' ' && mid_code[ mi ].res[ 0 ] != '\0' ) {
        if ( isdigit(mid_code[ mi ].res[ 0 ]) || mid_code[ mi ].op2[ 0 ] == '-' || mid_code[ mi ].op2[ 0 ] == '+' ) {
            fprintf(fp_final, "\t\tli\t$v0\t%s\n" ,mid_code[ mi ].res );
        } else {
            int addr2 = varaddr(mid_code[ mi ].res);
            if ( isglob )
...
void paraasm() {
	...
	int kind ;
    paranum = 0;
    for ( i = mi; i < midcode_num; i++ ) {
        if ( strcmp(mid_code[ i ].op, "para") == 0 )
            paranum++;
        else
            break;
    }
    for ( i = 0; i < paranum; i++ ) {
        kind = (strcmp(mid_code[ mi ].op1, "int") == 0) ? INT : CHAR;
...
//printf("%s %s %s %s:\n",mid_code[ mi ].op,mid_code[ mi ].op1,mid_code[ mi ].op2,mid_code[ mi ].res);
        pushstack(mid_code[ mi ].op2,1);
        if ( strcmp(mid_code[ mi ].op1, "int") == 0 ) {
            insertaddress(INT,-1,-1);
        } else {
            insertaddress(CHAR,-1,-1);
        }
...
void intcharaasm() {
    ...
    //数组定义
    while ( strcmp(mid_code[ mi ].op, "inta") == 0 || strcmp(mid_code[ mi ].op, "chara") == 0 ) {
        pushstack("0", atoi(mid_code[ mi ].op2));
        if ( strcmp(mid_code[ mi ].op, "inta") == 0 ) {
            insertaddress(INT,-1,-1);
        } else {
            insertaddress(CHAR,-1,-1);
        }
...
void error(char *name,int line,int error_type)
{
	...
	switch(error_type)
	{
        ...
        case 2:
			errclass = DONOTHING;
            fprintf(fp_error,"无主函数\n");
            break;
		case 3:
			errclass = DONOTHING;
            fprintf(fp_error,"主函数内部定义出错\n");
            break;
        case 4:										//OUTOFTABLE?????????????????在哪里出现？？？
            fprintf(fp_error,"%d行, 符号表空间不足\n",line);
			errclass = QUIT;
            break;
        case 6:    fprintf(fp_error,"%d行, %s语法上应为标识符/标识符不合法\n",line,name+1);
			errclass =
            signal == 0 ? ICV :
            signal == 1 ? CLR :
            signal == 2 ? IWFRSPIFCV :
            ...
            -1;
              break;
        case 7:    fprintf(fp_error,"%d行, 函数%s未定义\n",line,name+1);
			 errclass =
        ...
        case 9:    fprintf(fp_error,"%d行, %s标识符未定义\n",line,name+1);
			 errclass =
			 signal == 0 ?	IWFLIRSPE:
			 signal == 1 ? IWFXXXANE :
			 -1;
               break;
        case 10:   fprintf(fp_error,"%d行, 缺少类型符/关键词错误\n",line);//????????
				errclass = signal == 0 ? CICVIFIRSP :
				signal == 1 ? CLR :
                signal == 2 ? IWFRSPIFCV :
				-1;
               break;
        case 11:   fprintf(fp_error,"%d行, 缺少{\n",line);
			errclass =
            signal == 0 ? ICV :
            signal == 1 ? IWFLIRSPE :
            -1;
               break;
        case 12:   fprintf(fp_error,"%d行, 缺少}\n",line);
			errclass =
            signal == 0 ? ICV :
            signal == 1 ? IWFLIRSPE :
            -1;
               break;
        case 13:   fprintf(fp_error,"%d行, 缺少(\n",line);
				errclass =
            signal == 0 ? ICV :
            signal == 1 ? IWFLIRSPE :
            signal == 2 ? IWFXXXANE :
            ...
            -1;
               break;
        case 14:   fprintf(fp_error,"%d行, 缺少)\n",line);
			errclass =
            signal == 0 ? ICV :
            signal == 1 ? IWFLIRSPE :
            signal == 2 ? IWFXXXANE :
            -1;
               break;
        case 15:
				fprintf(fp_error,"%d行，标识符%s重复定义\n",line,name+1);
				errclass = DONOTHING;
               break;
        case 16:   fprintf(fp_error,"%d行，缺\"\n",line);
               break;
        case 17:   fprintf(fp_error,"语句形式出错\n");
				errclass =
				signal == 0 ?  IWFLIRSPE:
				signal == 1 ? IWFXXXANE :
				-1;
               break;
        case 18:   fprintf(fp_error,"%d行，常量未赋值\n",line);
               break;
        case 19:   fprintf(fp_error,"%d行, 赋值式左部非变量\n",line);
			errclass =
			signal == 0 ?  IWFLIRSPE:
            signal == 1 ? ICV :
			-1;
               break;
        case 20:   fprintf(fp_error,"%d行, 常量赋值式右部类型错误\n",line);
				errclass = CICVIFIRSP;
               break;
        case 21: fprintf(fp_error,"%d行, 应有‘=’\n",line);
			errclass =
			signal == 0 ?  IWFLIRSPE:
            signal == 1 ? ICV :
			-1;
               break;
        case 22: fprintf(fp_error,"%d行, %s不能是void型\n",line,name+1);
			errclass =
            signal == 0 ? ICV :
            signal == 1 ? IWFLIRSPE :
            signal == 2 ? IWFXXXANE :
            -1;
		     break;
        case 23:    fprintf(fp_error,"%d行, %s标识符类型错误\n",line,name+1);
			errclass =
			signal == 0 ?  IWFLIRSPE:
            signal == 1 ? ICV :
			-1;
               break;
        case 24: fprintf(fp_error,"%d行, 参数个数错误\n",line);
			errclass =
            signal == 0 ? ICV :
            signal == 1 ? IWFLIRSPE :
            signal == 2 ? IWFXXXANE :
            -1;
		     break;
		case 25: fprintf(fp_error,"%d行, 参数类型不匹配\n",line);
			errclass =
            signal == 0 ? ICV :
            signal == 1 ? IWFLIRSPE :
            signal == 2 ? IWFXXXANE :
            -1;
		     break;
        case 26: fprintf(fp_error,"%d行, 函数%s无返回语句\n",line,name+1);
			errclass =
			signal == 0 ? ICV :
            signal == 1 ? IWFLIRSPE :
			-1;
	   ...
	   /*case 27:printf("%d行, 赋值符号两边类型不匹配\n",line);
			 break;*/
	   case 28:fprintf(fp_error,"%d行, 数组定义出错,数组内部丢失立即数\n",line);
			 errclass = IWFRSPIFCV;
			 break;
	   ...
	   case 31:printf("%d行, 不允许在子函数中定义函数\n",line);
			 break;*/
	   case 32:fprintf(fp_error,"%d行, 此处缺少case\n",line);
			errclass =
			signal == 0 ?  IWFLIRSPE:
            signal == 1 ? ICV :
			-1;
	   ...
	   /*case 33:printf("%d行, 此处数组越界\n",line);
		...
		case 34:printf("%d行, 缺少[\n",line);
			 break;*/
		case 35:fprintf(fp_error,"%d行, 缺少]\n",line);
			errclass =
			signal == 0 ?  IWFLIRSPE:
            signal == 1 ? ICV :
			signal == 2 ? IWFXXXANE:
			-1;
		     break;
		case 36:fprintf(fp_error,"%d行,  缺少\n",line);//DOUBLEQUOTESLACK
			errclass = DONOTHING;
			break;
		case 37:fprintf(fp_error,"%d行,  不可接受的字符\n",line);//UNACCEPTATLECHAR
			errclass = DONOTHING;
			break;
		case 38:fprintf(fp_error,"%d行,  缺少'\n",line);//SINGLEQUOTESLACK
			errclass = DONOTHING;
			break;
		case 39:fprintf(fp_error,"%d行,  常量定义失败\n",line);//CSTDEFINEFAIL
			errclass = CICVIFIRSP;
			break;
		default:
            break;
...
//printf("begin!!\n");
	...
	fp4=fopen("siyuanshi2.txt","w");
	fp_error=fopen("errorrecord2.txt","w");
	//fp3=fopen("yufafenxi.txt","w");
	fp_final=fopen("assembly_code2.txt","w");
